...
async def calcular_totais_endpoint(divisao_id: str):
    ...
    # 4. Calcular o progresso da distribuição.
    itens_distribuidos = sum(1 for item in divisao.itens if sum(item.atribuido_a.values()) > 0)
    total_de_itens = len(divisao.itens)
    percentual_distribuido = (itens_distribuidos / total_de_itens) * 100 if total_de_itens > 0 else 0
    
    progresso = Progresso(
        percentual_distribuido=round(percentual_distribuido, 2),
        itens_restantes=total_de_itens - itens_distribuidos
    )
